...
\small\begin{ctabular}{lccl}
	...
	\signal{cont\_i} & in & 1 & Continue after \instr{hlt} \\
	\bottomrule
\end{ctabular}\normalsize
...
Receiving an enabled, non-blocked interrupt will make the CPU continue execution after being halted by the \instr{hlt} instruction. Alternatively, a high level on the \signal{cont\_i} port will make the CPU continue execution without invoking an interrupt handler. The effect is similar to invoking an interrupt with an empty handler (containing only \instr{iret}), but without the additional overhead of two jumps.
