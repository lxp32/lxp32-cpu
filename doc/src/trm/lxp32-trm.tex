...
\begin{enumerate}
	...
	\item For small programs, consider mapping code and data memory to the beginning or end of the address space (i.e. \code{0x00000000}--\code{0x0001FFFF} or \code{0xFFFE0000}--\code{0xFFFFFFFF}) to be able to load pointers with the \instr{lc18} instruction which saves both memory and CPU cycles as compared to \instr{lc}.
\end{enumerate}
...
\begin{ctabular}{lll}
	...
	\hyperref[subsec:instr:lc18]{\instr{lc18}} & Load Constant 18-bit & \code{000011} \\
...
Load a 32-bit word to the specified register. Note that values from the [-131072; 131071] range can be loaded more efficiently using the \instr{lc18} instruction. Values from the [-128; 127] range can be also loaded using the \instr{mov} instruction alias.
...
\subsection{\instr{lc18} -- Load Constant 18-bit}
\label{subsec:instr:lc18}

Load a signed 18-bit value to the specified register. Unlike the \instr{lc} instruction, this instruction is encoded as a single word.

\subsubsection{Syntax}

\code{\instr{lc18} DST, VAL}

\subsubsection{Encoding}

\code{000011 VAL[17:16] DST VAL[15:0]}

Example: \code{\instr{lc18} r1, -100000} $\rightarrow$ \code{0x0E017960}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{0.8\textwidth}{LLLL}
		...
		\instr{lc18} & 1 & \instr{sru} & 2  \\
		\instr{lsb} & $\ge$ 3\footnotemark[2] & \instr{sub} & 1 \\
		\instr{lub} & $\ge$ 3\footnotemark[2] & \instr{sw} & $\ge$ 2\footnotemark[2] \\
		\instr{lw} & $\ge$ 3\footnotemark[2] & \instr{xor} & 1 \\
		\instr{mods} & 37 & & \\
		\bottomrule
	\end{tabularx}
\end{table}

\footnotetext[1]{Depends on the instruction bus latency. Includes pipeline flushing overhead.}
\footnotetext[2]{Depends on the data bus latency.}
\footnotetext[3]{Depends on the multiplier architecture set with the \code{MUL\_ARCH} generic. See Section \ref{sec:generics}.}
...
Symbols can be used as operands to the \instr{lc} and \instr{lc18} instruction statements. A symbol reference can end with a \code{@\emph{n}} sequence, where \code{\emph{n}} is a numeric literal; in this case it is interpreted as an offset (in bytes) relative to the symbol definition. For the \instr{lc18} instruction, the resulting address must still fit into the signed 18-bit value range (\code{0x00000000}--\code{0x0001FFFF} or \code{0xFFFE0000}--\code{0xFFFFFFFF}), otherwise the linker will report an error.

To refer to symbols defined in other modules, they must first be declared external using the \instr{\#extern} directive.
...
\begin{itemize}
	\item A new instruction, \instr{lc18}, was added, which loads a signed 18-bit constant to a register. Unlike \instr{lc}, it is encoded as a single word and takes one cycle to execute.
	\item Optimizations in the divider unit. Division instructions (\instr{divs} and \instr{divu}) now take one less cycle to execute (modulo instructions are unaffected).
\end{itemize}
