...
\small\begin{ctabular}{lccl}
	...
	\signal{cont\_i} & in & 1 & Continue execution (see \ref{sec:interrupts}) \\
	\bottomrule
\end{ctabular}\normalsize
...
Receiving an enabled, non-blocked interrupt will make the CPU continue execution after being halted by the \instr{hlt} instruction. Alternatively, a high level on the \signal{cont\_i} port will make the CPU continue execution without invoking an interrupt handler. The effect is similar to invoking an interrupt with an empty handler (containing only \instr{iret}), but without the overhead of interrupt processing.

The \signal{cont\_i} signal does not affect the CPU when it is not halted. Like \signal{irq\_i}, \signal{cont\_i} must be synchronous with the system clock.
...
Halt the CPU until an interrupt is received, or the \signal{cont\_i} signal is asserted.
