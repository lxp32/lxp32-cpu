...
Procedure calls are supported in \lxp{} by the \instr{xcall} instruction which saves the address of the next instruction in the register specified as the first operand and transfers execution to the address pointed by the second operand. In a common case when the first operand is \code{rp}, the instruction can be also written as \instr{call} which takes only one operand.

Return from a procedure is performed by the \instr{jmp} instruction; \code{\instr{jmp} rp} also has a \instr{ret} alias.
...
\begin{codepar}
    ...
    \instr{call} r0 \emph{// equivalent to xcall rp, r0}
    ...
    \instr{lw} rp, sp
    \instr{add} sp, sp, 4
    \instr{ret} \emph{// equivalent to jmp rp}
\end{codepar}
...
The following generics can be used to configure the \lxp{} IP core.

\subsection{DBUS\_RMW}

This \code{boolean} generic controls how byte-granular data bus access is performed. \lxp{} byte-granular instructions are \instr{lub} (\instrname{Load Unsigned Byte}), \instr{lsb} (\instrname{Load Signed Byte}) and \instr{sb} (\instrname{Store Byte}).

Some WISHBONE buses provide hardware support for byte-granular access by the means of the [SEL\_I()] (byte enable) port. In this case this option should be left to \code{false}, which is the default value.

\lxp{} can also emulate byte-granular transactions. This works even if the data bus doesn't support byte-granular access natively. To enable this feature, DBUS\_RMW should be set to \code{true}. The emulation works like this:

\begin{itemize}
\item When executing the \instr{sb} instruction, the CPU performs an RMW (read-modify-write) cycle, reading the entire word, replacing a byte in it and writing the word back.
\item When executing the \instr{lub} and \instr{lsb} instructions, the CPU reads the entire word and extracts the requested byte.
\end{itemize}

Either way, when this option is set to \code{true}, \signal{dbus\_sel\_o} is tied to \code{"1111"}.

\subsection{DIVIDER\_EN}

\lxp{} includes a divider unit which has quite a low performance but occupies a considerable amount of resources. It can be disabled by setting this \code{boolean} generic to \code{false}.

\subsection{IBUS\_BURST\_SIZE}

This \code{integer} generic determines the instruction bus burst size. Default value is 16. Only for \lxp{}C.

\subsection{IBUS\_PREFETCH\_SIZE}

This \code{integer} generic determines the number of words that the instruction cache will read ahead from the current instruction pointer. Default value is 32. Only for \lxp{}C.

\subsection{MUL\_ARCH}

This \code{string} generic selects the multiplier hardware implementation. \lxp{} provides three multiplier options:
...
Address of the first instruction to be executed after CPU reset, represented as \code{std\_logic\_vector}. Default value is \code{0}. The two least significant bits are ignored as instructions are always word-aligned.
...
The data bus uses a 30-bit \signal{dbus\_adr\_o} port to address 32-bit words; the \signal{dbus\_sel\_o} port is used to select individual bytes to be written or read. The upper 30 bits of the address appear on the \signal{dbus\_adr\_o} port. \signal{dbus\_sel\_o} is set to \code{"1111"} when reading or writing whole words; for byte-granular transactions the lower two bits are decoded to create a 4-bit \signal{dbus\_sel\_o} signal. Consider:
...
\begin{itemize}
	...
	\begin{itemize}
		\item GHDL -- a free and open source VHDL simulator\footnote{\url{http://ghdl.free.fr/}};
		\item NVC -- another free and open source VHDL simulator\footnote{\url{https://www.nickg.me.uk/nvc/}};
	...
	With GHDL and NVC, a waveform viewer such as GTKWave is also recommended (Figure \ref{fig:gtkwave})\footnote{\url{http://gtkwave.sourceforge.net/}}.
...
\begin{itemize}
	...
	\item \shellcmd{gui} -- simulate the design in GUI mode. Note: with GHDL and NVC, the simulation itself will be run in batch mode; upon a completion, GTKWave will be run automatically to display the dumped waveforms.
...
It shouldn't be hard to create a script for a new simulator using one of the existing makefiles as an example.
...
\begin{itemize}
	\item \code{CPU\_DBUS\_RMW} -- \code{DBUS\_RMW} CPU generic value (see Section \ref{sec:generics}). Default is \code{false}.
	\item \code{CPU\_MUL\_ARCH} -- \code{MUL\_ARCH} CPU generic value (see Section \ref{sec:generics}). Default is \code{"dsp"}.
	\item \code{MODEL\_LXP32C} -- simulate the \lxp{}C version. Default is \code{true}. If set to \code{false}, \lxp{}U is simulated instead.
	\item \code{TEST\_CASE} -- if set to \code{"all"} (default), all tests are executed. Otherwise, specifies the test case to run.
	\item \code{THROTTLE\_DBUS} -- perform pseudo-random data bus throttling. Default is \code{true}.
	\item \code{THROTTLE\_IBUS} -- perform pseudo-random instruction bus throttling. Default is \code{true}.
	\item \code{VERBOSE} -- print more messages. Default is \code{false}.
\end{itemize}

These generics can be also set using \shellcmd{make} parameters (currently works only for \shellcmd{ghdl}, \shellcmd{nvc} and \shellcmd{vsim}), for example:

\begin{codepar}
    make MODEL_LXP32C=false TEST_CASE=test001
\end{codepar}
...
\begin{ctabular}{lll}
	...
	\hyperref[subsec:instr:call]{\instr{call}} & Call Procedure & alias for \code{\instr{xcall} rp, addr} \\
	\hyperref[subsec:instr:xcall]{\instr{xcall}} & Extended Call & \code{100001} \\
...
Alias for \code{\instr{xcall} rp, RD1}
...
\subsection{\instr{xcall} -- Extended Call}
\label{subsec:instr:xcall}

Save a pointer to the next instruction in the register specified as the first operand and transfer execution to the address pointed by the second operand.

\subsubsection{Syntax}

\code{\instr{xcall} DST, RD1}

\subsubsection{Encoding}

\code{100001 1 0 DST RD1 00000000}

RD1 must be a register.

Example: \code{\instr{xcall} rp, r1} $\rightarrow$ \code{0x86FE0100}

\subsubsection{Operation}

\code{DST := \emph{return\_address}}

\code{goto RD1}

Pointer in RD1 is interpreted as described in Section \ref{sec:addressing}. \code{\instr{xcall} rp, RD1} is equivalent to \code{\instr{call} RD1}.
...
Cycle counts for \lxp{} instructions are listed in Table \ref{tab:cycles}. These data are provided for reference purposes; the software should not depend on them as they can change in future hardware revisions.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{0.8\textwidth}{LLLL}
		...
		\midrule
		\instr{add} & 1 & \instr{mov} & 1 \\
		\instr{and} & 1 & \instr{mul} & 2, 6 or 34\footnotemark[3] \\
		\instr{call} & 4 & \instr{neg} & 1 \\
		\instr{cjmp\emph{xxx}} & 5 or 2\footnotemark[1] & \instr{nop} & 1 \\
		\instr{divs} & 36 & \instr{not} & 1 \\
		\instr{divu} & 36 & \instr{or} & 1 \\
		\instr{hlt} & N/A & \instr{ret} & 4 \\
		\instr{jmp} & 4 & \instr{sb} & $\ge$ 2\footnotemark[2] \\
		\instr{iret} & 4 & \instr{sl} & 2 \\
		\instr{lc} & 2 & \instr{srs} & 2 \\
		\instr{lcs} & 1 & \instr{sru} & 2 \\
		\instr{lsb} & $\ge$ 3\footnotemark[2] & \instr{sub} & 1 \\
		\instr{lub} & $\ge$ 3\footnotemark[2] & \instr{sw} & $\ge$ 2\footnotemark[2] \\
		\instr{lw} & $\ge$ 3\footnotemark[2] & \instr{xcall} & 4 \\
...
\section*{Version 1.5.1 (XXXX-XX-XX)}

This release introduces a few testbench improvements. Support for a new simulator, NVC, has been added. It is also now possible to override testbench generics with make command-line arguments.

\section*{Version 1.5 (2026-09-03)}

This release introduces support for a previously undocumented \instr{xcall} instruction to the toolchain. The RTL description is unchanged.
