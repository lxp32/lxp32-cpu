...
\begin{itemize}
	\item \shellcmd{-a \emph{align}} -- section alignment. Must be a power of 2 and can't be less than 4. Default value is 4. Ignored in compile-only mode.
	...
	\item \shellcmd{-m \emph{file}} -- generate map file. Ignored in compile-only mode.
...
Load a 32-bit word to the specified register. Note that values from the [-1048576; 1048575] range can be loaded more efficiently using the \instr{lcs} instruction. Values from the [-128; 127] range can be also loaded using the \instr{mov} instruction alias.
...
Load a signed value from the [-1048576; 1048575] range to the specified register. Unlike the \instr{lc} instruction, this instruction is encoded as a single word.
...
A special \code{entry} symbol is used to inform the linker about the program entry point if there are multiple input files. It does not have to be exported. If defined, this symbol must precede the first instruction or data definition statement in the module.
...
This release introduces a minor but potentially breaking hardware change: the START\_ADDR generic, which used to be 30-bit, has been for convenience extended to a full 32-bit word; the two least significant bits are ignored.
...
The instruction set architecture (ISA) is backwards compatible with the previous release, so code compiled for the previous version will still work.

Other notable changes include:

\begin{itemize}
	\item A new instruction, \instr{lcs}, has been added, which loads a 21-bit sign extended constant to a register. Unlike \instr{lc}, it is encoded as a single word and takes one cycle to execute.
	...
	\item LXP32 assembly language now supports a new instruction alias, \instr{neg}, which is equivalent to \code{\instr{sub} dst, 0, src}.
\end{itemize}
