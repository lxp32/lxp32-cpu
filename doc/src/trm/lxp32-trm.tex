...
\begin{itemize}
	...
	\item \code{TEST\_CASE} -- if set to \code{"all"} (default), all tests are executed. Otherwise, specifies the file name of a test case to run.
...
These generics can be set using \shellcmd{make} parameters (currently works only for \shellcmd{ghdl} and \shellcmd{vsim}), for example:

\begin{codepar}
    make MODEL_LXP32C=false TEST_CASE=test001.ram
\end{codepar}
