...
\section{Addressing and byte-granular access}

From a programmer's point of view, \lxp{} uses 32-bit addresses which refer to individual bytes for both code and data, regardless of whether the hardware actually supports byte-granular access or not.

The instruction bus, whether LLI or WISHBONE, doesn't support access to individual bytes and uses a 30-bit address port to address 32-bit words. The lower two bits of the 32-bit address are ignored for the purpose of addressing. Consider the following example:
...
The data bus addresses 32-bit words as well and supports byte-granular access with the \signal{dbus\_sel\_o} (byte enable) WISHBONE port. The upper 30 bits of the address appear on the \signal{dbus\_adr\_o} port, while the lower two bits are decoded to create a 4-bit \signal{dbus\_sel\_o} signal. Consider:
...
The byte-granular access feature is optional. If it is not needed, just leave the \signal{dbus\_sel\_o} port unconnected. You can also set the \code{DBUS\_RMW} generic to \code{true} to enable byte-granular access emulation using the read-modify-write (RMW) cycle, which works even if the interconnect or slave doesn't provide the [SEL\_I()] port (Section \ref{sec:generics}).
