...
The CPU pipeline can be stalled by instruction and data buses. There are no internal pipeline stalls, that is, an instruction can use the result of the previous one without introducing additional wait cycles.
...
Symbols can be used as operands to the \instr{lc} and \instr{lc18} instruction statements. A symbol reference can end with a \code{@\emph{n}} sequence, where \code{\emph{n}} is a numeric literal; in this case it is interpreted as an offset (in bytes) relative to the symbol definition. For the \instr{lc18} instruction, the resulting address must still fit into the signed 18-bit value range, otherwise the linker will report an error.
