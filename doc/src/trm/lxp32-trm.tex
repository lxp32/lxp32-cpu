...
If a procedure must in turn call a nested procedure itself, the return address in the \code{rp} register will be overwritten by the \instr{call} instruction. Hence the procedure must save its value somewhere; the most general solution is to use the stack:
...
It can be useful to adopt a naming convention to distinguish between various types of identifiers. For example, use upper case for macros (see below), capitalize the first letter for procedure labels and use lower case for other kinds of symbols.
