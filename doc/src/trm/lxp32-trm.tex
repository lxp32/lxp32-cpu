...
\begin{enumerate}
	...
	\item For small programs, consider mapping code and data memory to the beginning or end of the address space (i.e. \code{0x00000000}--\code{0x000FFFFF} or \code{0xFFF00000}--\code{0xFFFFFFFF}) to be able to load pointers with the \instr{lc21} instruction which saves both memory and CPU cycles as compared to \instr{lc}.
\end{enumerate}
...
\begin{itemize}
	...
	\item \shellcmd{-na} -- do not use instruction aliases such as \instr{mov}, \instr{ret}, \instr{not}.
...
\begin{ctabular}{lll}
	...
	\hyperref[subsec:instr:lc21]{\instr{lc21}} & Load Constant 21-bit & \code{101xxx} \\
...
Load a 32-bit word to the specified register. Note that values from the [-1048576; 1048575] range can be loaded more efficiently using the \instr{lc21} instruction. Values from the [-128; 127] range can be also loaded using the \instr{mov} instruction alias.
...
\subsection{\instr{lc21} -- Load Constant 21-bit}
\label{subsec:instr:lc21}

Load a signed 21-bit value (i.e. from the [-1048576; 1048575] range) to the specified register. Unlike the \instr{lc} instruction, this instruction is encoded as a single word.

\subsubsection{Syntax}

\code{\instr{lc21} DST, VAL}

\subsubsection{Encoding}

\code{101 VAL[20:16] DST VAL[15:0]}

Example: \code{\instr{lc21} r1, -1000000} $\rightarrow$ \code{0xB001BDC0}
...
Symbols can be used as operands to the \instr{lc} and \instr{lc21} instruction statements. A symbol reference can end with a \code{@\emph{n}} sequence, where \code{\emph{n}} is a numeric literal; in this case it is interpreted as an offset (in bytes) relative to the symbol definition. For the \instr{lc21} instruction, the resulting address must still fit into the signed 21-bit value range (\code{0x00000000}--\code{0x000FFFFF} or \code{0xFFF00000}--\code{0xFFFFFFFF}), otherwise the linker will report an error.
...
\begin{itemize}
	\item A new instruction, \instr{lc21}, was added, which loads a signed 21-bit constant to a register. Unlike \instr{lc}, it is encoded as a single word and takes one cycle to execute.
